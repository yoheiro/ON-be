...
*/
(function () {
  const PLATFORM_VERSION_BUILD_LABEL = '8.1.0';
  ...
  define('cordova/exec', function (require, exports, module) {
    ...
    const nativeApiProvider = require('cordova/android/nativeapiprovider');
    const utils = require('cordova/utils');
    ...
    let jsToNativeBridgeMode; // Set lazily.
    let nativeToJsBridgeMode = nativeToJsModes.EVAL_BRIDGE;
    let pollEnabled = false;
    let bridgeSecret = -1;

    const messagesFromNative = [];
    let isProcessing = false;
    const resolvedPromise =
      typeof Promise === 'undefined' ? null : Promise.resolve();
    const nextTick = resolvedPromise
      ? function (fn) {
    ...
    function buildPayload(payload, message) {
      ...
      if (payloadKind == 's') {
        ...
        let multipartMessages = message.slice(1);
        while (multipartMessages !== '') {
          const spaceIdx = multipartMessages.indexOf(' ');
          const msgLen = +multipartMessages.slice(0, spaceIdx);
          const multipartMessage = multipartMessages.substr(
            spaceIdx + 1,
            msgLen
          );
    ...
    function popMessageFromQueue() {
      let messageBatch = messagesFromNative.shift();
      if (messageBatch == '*') {
        return '*';
      }

      const spaceIdx = messageBatch.indexOf(' ');
      const msgLen = +messageBatch.slice(0, spaceIdx);
      const message = messageBatch.substr(spaceIdx + 1, msgLen);
  ...
  define('cordova/modulemapper', function (require, exports, module) {
    ...
    const moduleMap = define.moduleMap; // eslint-disable-line no-undef
    let symbolList;
    let deprecationMap;
  ...
  define('cordova/pluginloader', function (require, exports, module) {
    ...
    function handlePluginsObject(path, moduleList, finishPluginLoading) {
      ...
      let scriptCounter = moduleList.length;
    ...
    exports.load = function (callback) {
      let pathPrefix = findCordovaPath();
      if (pathPrefix === null) {
        console.log(
