...
const builders = require('./builders/builders');
...
function parseOpts(options, resolvedTarget, projectRoot) {
  ...
  if (!ret.packageInfo) {
    ...
    for (const key in packageArgs) {
      if (!shouldWarn && signingKeys.includes(key)) {
  ...
  if (packageArgs.packageType) {
    ...
    if (!VALID_PACKAGE_TYPES.includes(packageArgs.packageType)) {
      events.emit(
        'warn',
        '"' +
...
module.exports.runClean = function (options) {
  ...
  const builder = builders.getBuilder();
