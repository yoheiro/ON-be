    ...
    for (let i = 1; i < response.length; i++) {
      ...
      if (response[i].match(/Name:\s/)) {
        ...
        if (
          ...
          i++;
          const secondLine = response[i + 1].match(/\(API\slevel\s/)
            ? response[i + 1]
            : '';
  ...
  return Promise.resolve()
    ...
    .then(function (avds) {
      ...
      return avds.map(function (avd) {
        if (
          avd.target &&
          avd.target.includes('Android API') &&
          !avd.target.includes('API level')
        ) {
...
module.exports.list_targets = function () {
  return superspawn
    ...
    .then(function (output) {
      ...
      const targets = [];
      for (let i = target_out.length; i >= 0; i--) {
...
module.exports.get_available_port = function () {
  ...
  return self.list_started().then(function (emulators) {
    for (let p = 5584; p >= 5554; p -= 2) {
      if (!emulators.includes('emulator-' + p)) {
  ...
  return Promise.resolve()
    .then(function () {
      ...
      return self.best_image().then(function (best) {
        ...
        const androidCmd = check_reqs.getAbsoluteAndroidCmd();
        return Promise.reject(
          new CordovaError(
    ...
    .then(function (emulatorId) {
      return self.get_available_port().then(function (port) {
        ...
        const emulator_dir = path.dirname(shelljs.which('emulator'));
    ...
    return Adb.shell(emulator_id, 'getprop dev.bootcomplete').then(
      ...
      function (error) {
        if (
          ...
          error.message.includes('device still connecting') ||
          error.message.includes('device still authorizing')
        ) {
...
module.exports.resolveTarget = function (target) {
  return this.list_started().then(function (emulator_list) {
    ...
    if (!emulator_list.includes(target)) {
      return Promise.reject(
        new CordovaError(
    ...
    return build.detectArchitecture(target).then(function (arch) {
      return { target, arch, isEmulator: true };
    });
  });
};
...
module.exports.install = function (givenTarget, buildResults) {
  ...
  return Promise.resolve()
    ...
    .then(function () {
      ...
      return Promise.resolve().then(function () {
        const apk_path = build.findBestApkForArchitecture(
          buildResults,
          target.arch
        );
        const execOptions = {
        ...
        function adbInstallWithOptions(target, apk, opts) {
          ...
          const command = 'adb -s ' + target + ' install -r "' + apk + '"';
