...
function updateProjectAccordingTo(platformConfig, locations) {
  ...
  const javaPattern = path.join(
    locations.javaSrc,
    manifestId.replace(/\./g, '/'),
    '*.java'
  );
  const java_files = shell.ls(javaPattern).filter(function (f) {
...
function updateSplashes(cordovaProject, platformResourcesDir) {
  ...
  const resourceMap = mapImageResources(
    cordovaProject.root,
...
<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    const launcherXmlPath = path.join(
      platformResourcesDir,
      'mipmap-' + density + '-v26',
      'ic_launcher.xml'
    );
...
function updateIconResourceForLegacy(
  ...
  // There's no "default" drawable, so assume default == mdpi.
  if (default_icon && !android_icons.mdpi) {
    const defaultTargetPath = getImageResourcePath(
      platformResourcesDir,
      'mipmap',
      'mdpi',
      'ic_launcher.png',
...
function findAndroidLaunchModePreference(platformConfig) {
  ...
  const valid = expectedValues.includes(launchMode);
  if (!valid) {
