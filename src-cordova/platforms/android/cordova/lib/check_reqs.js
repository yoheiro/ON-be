...
module.exports.get_target = function () {
  function extractFromFile(filePath) {
    const target = shelljs.grep(/\btarget=/, filePath);
    if (!target) {
      throw new Error('Could not find android target within: ' + filePath);
    }
    return target.split('=')[1].trim();
  }
  const repo_file = path.join(REPO_ROOT, 'framework', 'project.properties');
  if (fs.existsSync(repo_file)) {
    return extractFromFile(repo_file);
  }
  const project_file = path.join(PROJECT_ROOT, 'project.properties');
...
module.exports.check_android = function () {
  return Q().then(function () {
    const androidCmdPath = forgivingWhichSync('android');
    const adbInPath = forgivingWhichSync('adb');
    const avdmanagerInPath = forgivingWhichSync('avdmanager');
    let hasAndroidHome =
      !!process.env.ANDROID_HOME && fs.existsSync(process.env.ANDROID_HOME);
...
module.exports.getAbsoluteAndroidCmd = function () {
  let cmd = forgivingWhichSync('android');
  if (cmd.length === 0) {
...
module.exports.check_android_target = function (originalError) {
  ...
  const desired_api_level = module.exports.get_target();
  return android_sdk.list_targets().then(function (targets) {
    if (targets.includes(desired_api_level)) {
      return targets;
    }
    const androidCmd = module.exports.getAbsoluteAndroidCmd();
    let msg =
